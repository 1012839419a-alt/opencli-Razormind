...
async def dispatch_scheduled_agent_run(run_id: str) -> ApiResponse:
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            raise HTTPException(
                status.HTTP_409_CONFLICT,
                "Published Operations Agent runtime binding is invalid",
            ) from exc
        if binding is None or not ws_agent_manager.is_connected(binding.agent_url):
            raise HTTPException(
                status.HTTP_503_SERVICE_UNAVAILABLE,
                "Bound Agent Runtime is not connected",
            )
