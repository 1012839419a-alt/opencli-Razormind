...
def main():
    ...
    # Search keyword is accepted for CLI compatibility; the URL is already set.
    parser.add_argument('keyword')
    parser.add_argument('--page', default='1')              # page number, 1-based
    # Sort: '', 'sale-desc', 'price-asc', or 'price-desc'.
      ...
      try {
        ...
        if (!cards || cards.length === 0) {
          return JSON.stringify({ error: true, message:
            'No product cards found. Page may not have loaded or login is required.' });
        }
        var items = Array.from(cards).map(function(card) {
          var a = card.tagName === 'A' ? card : card.querySelector(
            'a[href*="item.taobao.com"], a[href*="detail.tmall.com"]');
          var href = a ? a.href : '';
          var url;
          try { url = new URL(href || 'https://taobao.com'); }
          catch(e) { url = new URL('https://taobao.com'); }
          ...
          var tags = Array.from(card.querySelectorAll(
            '[class*="tag--"], [class*="Tag--"], [class*="label--"]'))
            .map(function(t) { return t.textContent.trim(); })
            .filter(function(t) { return t.length > 0; })
            .slice(0, 5);
          var rawId = url.searchParams.get('id') || card.id.replace('item_id_', '');
          var rawHref = href.indexOf('simba.taobao.com') >= 0 ? href :
            (href.split('?')[0] + (rawId ? '?id=' + rawId : ''));
          return {
            itemId: rawId,
            itemUrl: rawHref,
            title: (titleEl ? titleEl.textContent.trim() : (img ? img.alt : '')) || '',
            subTitle: subTitle ? subTitle.textContent.trim() : '',
            priceYuan: (priceInt && priceFloat)
              ? parseFloat(priceInt.textContent.trim() + priceFloat.textContent.trim())
              : null,
