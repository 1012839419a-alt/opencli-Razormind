...
def run_google_image_task(
    api_key, keywords, country="us", language="en", scroll_count=5, datelimit=50
):
    ...
    while time.time() - poll_start < max_poll_time:
        ...
        time.sleep(10)
    print(
        f"Error: Task polling timed out after {max_poll_time} seconds.", flush=True
    )
    return None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python google_image_api.py <keywords> [country] [language] "
            "[scroll_count] [datelimit]",
            flush=True,
        )
        sys.exit(1)
    ...
    result = run_google_image_task(
        api_key, keywords, country, language, scroll_count, datelimit
    )
    if result:
