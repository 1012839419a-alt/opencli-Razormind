...
# Force UTF-8 encoding for standard output and error streams to ensure
...
def run_google_news_task(api_key, keywords, date_range="past week", limit=30):
    ...
    Args:
        ...
        date_range (str): Time range filter (any time, past hours, past 24
            hours, past week, past year)
    ...
    if response.status_code == 401 or (
        isinstance(res, dict) and "Invalid authorization" in str(res)
    ):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print(
            'Example: python google_news_api.py "AI technology" "past 24 hours" 10',
            flush=True,
        )
        sys.exit(1)
    ...
    if not api_key:
        ...
        print(
            "3. Set it as an environment variable (BROWSERACT_API_KEY) or "
