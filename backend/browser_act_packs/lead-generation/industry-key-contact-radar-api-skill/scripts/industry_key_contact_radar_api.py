...
def run_industry_key_contact_radar_task(
    api_key,
    industry,
    date_limit=10,
    site="facebook.com",
    ...
    # 1. Start Task
    print("Starting task via BrowserAct API...", flush=True)
    try:
    ...
    print("Waiting for task completion...", flush=True)
    max_poll_time = 900
    ...
    # 3. Get Results
    print("Retrieving results...", flush=True)
    try:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python industry_key_contact_radar_api.py <industry> "
            "[date_limit] [site] [job_title]",
            flush=True,
        )
        print(
            'Example: python industry_key_contact_radar_api.py "Browser automation" '
            '10 "facebook.com" "founder"',
            flush=True,
        )
        sys.exit(1)
    ...
    if not api_key:
        ...
        print(
            "3. Set it as an environment variable (BROWSERACT_API_KEY) or provide it "
            "in the chat.",
            flush=True,
        )
        sys.exit(1)
    ...
    result = run_industry_key_contact_radar_task(
        api_key, industry, date_limit, site, job_title
    )
    if result:
