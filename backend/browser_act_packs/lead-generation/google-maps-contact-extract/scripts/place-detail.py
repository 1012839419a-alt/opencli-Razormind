...
(function() {
  try {
    ...
    var isPlacePage = url.indexOf('/maps/place/') !== -1 ||
      url.indexOf('place_id') !== -1 || placeId;
    if (!isPlacePage) {
      return JSON.stringify({
        error: true,
        message: 'Not on a place detail page. Navigate to a Google Maps place URL first.'
      });
    }
    ...
    if (!name) {
      return JSON.stringify({
        error: true,
        message: 'Place detail not loaded yet. Make sure h1 is visible before running.'
      });
    }

    var ratingEl = document.querySelector('[aria-label$=" stars "]') ||
    ...
    var hoursRows = Array.from(document.querySelectorAll('tr'))
      ...
      .filter(function(t) {
        return t.match(/Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday/);
      });
    var hours = hoursRows.length > 0 ? hoursRows : null;

    var serviceOpts = Array.from(document.querySelectorAll(
      '[aria-label*="ine-in"],[aria-label*="akeout"],[aria-label*="elivery"],[aria-label*="ickup"]'
    )).map(function(el) { return el.getAttribute('aria-label'); });

    var aboutPattern = new RegExp(
      '^(Has|Accepts|No |Good for|Popular for|Casual|Cozy|Trendy|Upscale|' +
      'Quick bite|Family|Serves great|Has great)'
    );
    var aboutLabels = Array.from(document.querySelectorAll('[aria-label]'))
      .map(function(el) { return el.getAttribute('aria-label'); })
      .filter(function(l) {
        return l && l.length > 5 && l.length < 80 && aboutPattern.test(l);
      });
