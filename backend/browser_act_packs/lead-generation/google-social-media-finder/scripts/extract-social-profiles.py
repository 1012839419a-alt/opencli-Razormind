    ...
    (function() {
      try {
        var socialDomains = [
          ...
          'snapchat.com', 'threads.net', 'weibo.com', 'vk.com', 'discord.com',
          'twitch.tv', 'github.com', 'medium.com', 'quora.com', 'tumblr.com',
          'soundcloud.com'
        ];
        ...
        function isSocialDomain(url) {
          try {
            ...
            return socialDomains.some(function(d) {
        ...
        if (!containers.length) {
          return JSON.stringify({
            error: true,
            message:
              'No search result containers found (.tF2Cxc). Page may not have loaded ' +
              'or Google changed its layout.'
          });
        }
        ...
        containers.forEach(function(el) {
          ...
          var platformUser = el.querySelector('.VuuXrf')
            ? el.querySelector('.VuuXrf').textContent.trim()
            : '';
          var parts = platformUser.split(/\\s*\\u00b7\\s*/);

          var title = el.querySelector('h3.LC20lb')
            ? el.querySelector('h3.LC20lb').textContent.trim()
            : null;
          var snippet = el.querySelector('.VwiC3b')
            ? el.querySelector('.VwiC3b').textContent.trim()
            : null;
          var followers = el.querySelector('cite.qLRx3b')
            ? el.querySelector('cite.qLRx3b').textContent.trim()
            : null;
