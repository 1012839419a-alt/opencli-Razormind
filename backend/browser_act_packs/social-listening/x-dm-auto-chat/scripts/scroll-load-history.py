...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            'Scroll message list to top to trigger loading older messages; '
            'returns whether new messages were loaded'
        )
    )
    parser.parse_args()
    ...
    (async () => {
      try {
        ...
        return JSON.stringify({
          ...
          reached_top: afterCount === beforeCount
            // No new messages loaded → likely at top
        });
      } catch(e) {
