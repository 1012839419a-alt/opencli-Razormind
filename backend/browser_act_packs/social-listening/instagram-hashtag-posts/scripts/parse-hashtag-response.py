...
def main():
    ...
    parser.add_argument(
        "response_file",
        help="Path to file containing raw JSON response body from network capture",
    )
    ...
    (function() {{
      try {{
        ...
        var graph = data.data && data.data.xdt_fbsearch__top_serp_graphql;
        var edges = graph && graph.edges || [];
        var pageInfo = graph && graph.page_info || {{}};
        var items = [];
