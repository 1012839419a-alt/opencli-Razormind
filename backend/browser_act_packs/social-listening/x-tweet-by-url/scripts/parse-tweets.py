...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            'Parse a single GraphQL response body captured via '
            '`browser-act network request <id>` into a normalized tweet list '
            'with cursor. Pass --source to indicate which endpoint produced it.'
        )
    )
    parser.add_argument(
        '--json-file',
...
def _extract(data, source):
    ...
    try:
        ...
        if source == 'search':
            ...
            timeline_root = (
                ((d.get('list') or {}).get('tweets_timeline') or {}).get('timeline')
                or {}
            )
