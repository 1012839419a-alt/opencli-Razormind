...
```text
部署 → 安装器生成密码并初始化 → 打开页面 → admin/随机密码登录 → 配置采集
```
...
- 本地管理员默认账号：`admin`
- 本地管理员初始密码：由安装器随机生成并在安装结束时显示
- 登录页只展示用户名和密码
- 登录成功后创建本地会话
- 登录页提示初始密码应在设置中修改
- 设置页提供修改本地管理员密码的入口
- 修改密码后持久化到服务端专用认证状态文件
...
1. 安装器通过 API 镜像在持久化 `db_data` 中一次性创建密码哈希和初始化标记。
2. 用户打开 `/login`。
3. 页面预填用户名 `admin`，密码由安装结束时显示的随机密码填写。
4. 登录成功后进入控制台。
5. 控制台账户菜单或设置页显示“建议修改初始密码”提示。
6. 用户可以稍后处理，不阻塞采集工具使用。
...
```json
{"username":"admin","password":"<installer-generated-password>"}
```
