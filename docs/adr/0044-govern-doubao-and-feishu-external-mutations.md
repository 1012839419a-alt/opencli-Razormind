...
- Conversation cleanup occurs only after the complete visible response, links, sharing metadata,
  ...
  only then sends an application-level `persisted` acknowledgement. The Agent waits for that exact
  event acknowledgement before cleanup; delivery failure, persistence failure, disconnect, or
  acknowledgement timeout all abort iteration before deletion. Unconfirmed cleanup fails closed and
  stops the remaining batch; an already captured successful prefix remains available to the workflow.
...
- A bridge token is mandatory for non-loopback listeners. The bridge accepts only allowlisted
  spreadsheet targets and verifies every appended row by address and idempotency value.
