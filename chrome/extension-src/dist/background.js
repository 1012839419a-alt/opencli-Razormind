...
async function evaluate(tabId, expression, aggressiveRetry = false) {
	...
	for (let attempt = 1; attempt <= MAX_EVAL_RETRIES; attempt++) try {
		...
		if ((msg.includes("Inspected target navigated") || msg.includes("Target closed") || msg.includes("attach failed") || msg.includes("Debugger is not attached")) && attempt < MAX_EVAL_RETRIES) {
			attached.delete(tabId);
...
async function getAutomationWindow(workspace, initialUrl) {
	...
	if (win?.id === void 0) throw new Error("Chrome did not create an automation window");
...
async function resolveTab(tabId, workspace, initialUrl) {
	...
	if (existingSession?.preferredTabId !== null && existingSession?.preferredTabId !== void 0) try {
...
async function handleCookies(cmd) {
	if (!cmd.domain && !cmd.url) return {
		id: cmd.id,
		ok: false,
		error: "Cookie scope required: provide domain or url"
	};
	const details = {};
