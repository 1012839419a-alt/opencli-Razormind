...
async def test_sqlite_raw_evidence_mutations_are_rejected_after_durable_execution(
    ...
    await db_session.execute(
        text(
            ...
            "WHEN NEW.attempt_number NOT BETWEEN 1 AND 3 OR "
            "NEW.outcome NOT IN ('accepted','rejected','unknown') "
    ...
    for event in ("INSERT", "UPDATE"):
        ...
        await db_session.execute(
            text(
                ...
                "ON controlled_receiver_deliveries WHEN NEW.durable_status "
    ...
    for attempt, outcome in ((0, "unknown"), (4, "unknown"), (1, "invalid")):
        await rejected(
            ...
            "VALUES (:id, :now, :now, :execution_id, :attempt, 'test', "
            "'missing', 'unknown', :outcome, :now)",
            {
                "id": str(uuid.uuid4()),
                "now": now,
    ...
    await rejected(
        ...
        "(id, created_at, updated_at, receiver_identity, operation_id, "
        "decision_hash, payload_hash, request_hash, "
        "durable_status, receipt_id, receipt_timestamp, receipt_key_id, "
        "receipt_signature) "
        "VALUES (:id, :now, :now, 'receiver-a', 'invalid-status', :hash, "
        ":hash, :hash, 'invalid', :receipt, :now, 'receipt-a', 'signature')",
        {
            "id": str(uuid.uuid4()),
            "now": now,
            "hash": "f" * 64,
...
async def test_sqlite_final_links_reject_cross_execution_and_missing_evidence(db_session):
    ...
    for column, valid_id, table in (
        ...
        await db_session.execute(
            text(
                f"CREATE TRIGGER link_guard_{column} BEFORE UPDATE OF "
                f"{column} ON delivery_executions "
