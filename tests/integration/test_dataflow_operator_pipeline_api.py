...
async def test_data_operator_failure_event_is_redacted_after_json_serialization(
    ...
    sensitive_marker = "candidate-value-must-not-leak"
    monkeypatch.setattr(
        "backend.workflow.opencli_hda_tracer.execute_data_operator",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError(sensitive_marker)),
    )
    ...
    assert sensitive_marker not in str(serialized_events)
