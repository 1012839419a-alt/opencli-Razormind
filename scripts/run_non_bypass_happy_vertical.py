...
def _migrate_fresh_admin(ledger: RunLedger, env: dict[str, str]) -> None:
    ...
    _compose(
        ledger, env, "run", "--rm", "--no-deps", "proof-admin",
        "/bin/sh", "-c", "PYTHONUNBUFFERED=1 alembic upgrade g5b6c7d8e9f0",
    )
    _compose(
        ledger, env, "exec", "-T", "proof-admin-postgres",
        "psql", "-U", "proof", "-d", "proof_admin", "-c",
    ...
    _compose(
        ledger, env, "run", "--rm", "--no-deps", "proof-admin",
        "/bin/sh", "-c", "PYTHONUNBUFFERED=1 alembic upgrade head",
    )
    canonical = _compose(
        ledger, env, "exec", "-T", "proof-admin-postgres",
        "psql", "-U", "proof", "-d", "proof_admin", "-At", "-c",
    ...
    _compose(
        ledger, env, "run", "--rm", "--no-deps", "proof-controlled-receiver",
        "/bin/sh", "-c", "PYTHONUNBUFFERED=1 alembic upgrade head",
    )
