...
const collectorSourceSchema = jsonRecordSchema.superRefine((source, context) => {
  ...
  for (const key of Object.keys(source)) {
    if (!COLLECTOR_SOURCE_FIELDS[parsedKind.data].has(key)) {
      context.addIssue({
        code: "custom",
        message: `Collector ${parsedKind.data} source field "${key}" is not editable by Agent`,
      })
    }
  }
  ...
  z.object({
    ...
    for (const key of Object.keys(operation.changes)) {
      if (!(key in operation.expected)) {
        context.addIssue({
          code: "custom",
          message: `Collector updateSource requires expected state for "${key}"`,
        })
      }
    }
  }),
...
export function acceptCollectorNodeProposal(
  ...
  return {
    ...
    proposal: actualRevision === proposal.baseRevision
      ? proposal
      : { ...proposal, baseRevision: actualRevision },
    differences: evaluation.differences,
    conflicts: [],
    changed,
  }
...
function applyCollectorOperation(
  ...
  switch (operation.type) {
    ...
    case "removeSource": {
      const sourceIndex = sources.findIndex((candidate) => candidate.sourceId === operation.sourceId)
      if (sourceIndex < 0) {
        return changed(
          `/nodes/${node.id}/params/sources/${operation.sourceId}`,
          undefined,
...
function collectorSources(node: WorkflowProjectNode): Record<string, unknown>[] {
  if (node.params.sources === undefined) node.params.sources = []
  ...
  return node.params.sources.map((source) => {
    ...
    if (expectedKind && parsed.kind !== expectedKind) {
      throw new Error(
        `Collector ${expectedKind} node "${node.id}" contains a ${String(parsed.kind)} source`,
      )
    }
...
function collectorNodeKind(
  node: WorkflowProjectNode,
): z.infer<typeof collectorSourceKindSchema> | undefined {
...
function validateExecutionValue(
  field: "concurrency" | "timeoutMs" | "retry",
  value: unknown,
): void {
  if (field === "retry") {
    z.object({
  ...
  for (const [key, nested] of Object.entries(value)) {
    if (FORBIDDEN_COLLECTOR_KEYS.has(normalizeCollectorKey(key))) {
      context.addIssue({
        code: "custom",
        path: [...path, key],
        message: `Sensitive or executable field "${key}" is forbidden`,
      })
    }
