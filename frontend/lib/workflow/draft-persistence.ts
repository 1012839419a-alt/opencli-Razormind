...
export function preparePersistableWorkflowDraft(project: WorkflowProject) {
  const graph = persistableWorkflowProject(project)
  return { graph, fingerprint: JSON.stringify(graph) }
}

export const workflowDraftFingerprint = (project: WorkflowProject) =>
  preparePersistableWorkflowDraft(project).fingerprint
