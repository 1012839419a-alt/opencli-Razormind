...
const CONTRACTS: Record<string, NodeContract> = {
  ...
  "intelligence.data.generate": contract(
    "intelligence.data.generate",
    "Generate Data",
    "recordCandidate[] -> recordCandidate[]",
    [port("in", "input", "recordCandidate[]", true, "Consumes record candidates.")],
    [port("out", "output", "recordCandidate[]", true, "Emits generated record candidates.")],
    [
      param("operatorId", "params", "string", true, "core.generate.instruction-pairs", { description: "Backend data operator id." }),
      param("instructionTemplate", "params", "string", false, "Use the source material to answer this request: {title}", { description: "Instruction template; supports {title} and {content}." }),
    ],
    ["runtime projection owns operator execution and the authoritative contract"],
  ),
  "intelligence.data.filter": contract(
    "intelligence.data.filter",
    "Filter Data",
    "recordCandidate[] -> recordCandidate[]",
    [port("in", "input", "recordCandidate[]", true, "Consumes record candidates.")],
    [port("out", "output", "recordCandidate[]", true, "Emits candidates passing the configured filter.")],
    [
      param("operatorId", "params", "string", true, "core.filter.quality", { description: "Backend data operator id." }),
      param("requiredFields", "params", "string[]", false, [], { description: "Fields that must be present and non-empty." }),
      param("minLength", "params", "number", false, 0, { min: 0, description: "Minimum text length." }),
      param("maxLength", "params", "number", false, undefined, { min: 0, description: "Optional maximum text length." }),
      param("blocklist", "params", "string[]", false, [], { description: "Blocked terms that reject a candidate." }),
      param("textField", "params", "string", false, "content", { description: "Candidate field used for length and blocklist checks." }),
    ],
    ["runtime projection owns operator execution and the authoritative contract"],
  ),
  "intelligence.data.evaluate": contract(
    "intelligence.data.evaluate",
    "Evaluate Data",
    "recordCandidate[] -> recordCandidate[]",
    [port("in", "input", "recordCandidate[]", true, "Consumes record candidates.")],
    [port("out", "output", "recordCandidate[]", true, "Emits evaluated record candidates.")],
    [
      param("operatorId", "params", "string", true, "core.evaluate.quality", { description: "Backend data operator id." }),
      param("minLength", "params", "number", false, 20, { min: 0, description: "Minimum text length used by quality evaluation." }),
      param("maxLength", "params", "number", false, undefined, { min: 0, description: "Optional maximum text length used by quality evaluation." }),
    ],
    ["runtime projection owns operator execution and the authoritative contract"],
  ),
  "intelligence.data.refine": contract(
    "intelligence.data.refine",
    "Refine Data",
    "recordCandidate[] -> recordCandidate[]",
    [port("in", "input", "recordCandidate[]", true, "Consumes record candidates.")],
    [port("out", "output", "recordCandidate[]", true, "Emits refined record candidates.")],
    [
      param("operatorId", "params", "string", true, "core.refine.text", { description: "Backend data operator id." }),
      param("fields", "params", "string[]", false, ["title", "content", "text", "instruction", "response"], { description: "Text fields to normalize." }),
      param("unicodeForm", "params", "string", false, "NFKC", { enum: ["NFC", "NFD", "NFKC", "NFKD"], description: "Unicode normalization form." }),
      param("redactEmail", "params", "boolean", false, false, { description: "Redact email addresses while refining text." }),
      param("redactPhone", "params", "boolean", false, false, { description: "Redact phone numbers while refining text." }),
    ],
    ["runtime projection owns operator execution and the authoritative contract"],
  ),
