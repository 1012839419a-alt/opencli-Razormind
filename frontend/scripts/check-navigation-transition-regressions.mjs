...
test('studio uses ordinary creation choices while Agent stays global', async () => {
  ...
  assert.match(agentDock, /proposalQueryKeys\(proposalToConfirm\)/)
  assert.match(agentDock, /queryClient\.invalidateQueries\(\{ queryKey \}\)/)
  assert.doesNotMatch(agentDock, /queryClient\.invalidateQueries\(\s*\)/)
})
